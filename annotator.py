"""
Github link : https://github.com/haochenheheda/segment-anything-annotator/tree/master
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None, global_w=1000, global_h=1800, model_type='vit_b', keep_input_size=True, max_size=1080):
        ...
        self.current_output_dir = r'output'
        ...
        self.category_list = [i.strip() for i in open('C:\\Users\P301644\Documents\GitHub\segment-anything-annotator-master\segment-anything-annotator-master\categories.txt', 'r', encoding='utf-8').readlines()]
        self.labelDialog = LabelDialog(
            parent=self,
    ...
    def saveLabels(self, filename):
        lf = LabelFile()
        masks_path = r'output'
        output = open(os.path.join(masks_path, 'tile_8.pkl'), 'wb')
        pickle.dump(self.anns, output)
        def format_shape(s):
    ...
    def clickManualSegBox(self):
        ...
        target_idx = np.argmax(iou_preds)
        self.show_proposals(masks,0)
        self.sam_mask_proposal = []
        # Serialize predictions and store in MaskData
        data = MaskData(
            masks=masks,#.reshape(3, -1),
            iou_preds=iou_preds,
        )
        # del masks
        ...
        # Compress to RLE
        data["masks"] = uncrop_masks(data["masks"], [0, 0, rh, rw], rh, rw)
    ...
    def addSamMask(self):
        ...
        self.anns[0]._stats['label'] = label
